...
| `engine` | web, images, news, videos, cards | `brave`, `mojeek`, `marginalia`, `staan` oder `all`; mehrere durch Komma oder Leerzeichen trennen | `all` |
...
- **`engine`** — ein unbekannter Suchmaschinenname gibt `400` mit der Liste gültiger Namen zurück. Medien-Tabs mischen ihren ergänzenden Anbieter (Pixabay / World News / Sepia) über Brave; für die reinen Web-Suchmaschinen (`mojeek`, `marginalia` und `staan`) ist dieser ergänzende Anbieter die einzige Medienquelle.
