...
```bash
curl -H "X-Api-Key: searpa_sk_<préfixe>.<secret>" \
  "https://search.example.com/api/v1/web/?q=climat"
```
...
HEADERS = {"Authorization": "Api-Key searpa_sk_<préfixe>.<secret>"}
...
> [!WARNING]
> Gardez les clés secrètes ; quiconque en possède une peut rechercher à votre place et consommer votre limite de débit. Le préfixe de marque `searpa_sk_` permet aux scanners de secrets de détecter une clé compromise. Révoquez une clé compromise et émettez-en une nouvelle.
