...
Seurch est conçu pour être une alternative privée aux moteurs de recherche financés par la surveillance. Voici ce que cela signifie en pratique.

## Ce que Seurch ne fait pas

- **Pas de traçage.** Seurch ne vous suit pas sur le web ni d'une session à l'autre pour construire un profil.
...
- Les recherches s'exécutent **côté serveur**. Seurch parle aux fournisseurs amont (Brave, Mojeek, Marginalia et les autres) en votre nom, de sorte que ces fournisseurs ne voient pas votre navigateur ni votre IP, ils voient le serveur de Seurch.
- **Les clés API des fournisseurs n'atteignent jamais votre navigateur.** Tous les appels amont se font sur le serveur.
- **Le proxy d'images** (optionnel, dans [Paramètres]({{< relref "settings" >}})) achemine les miniatures via le serveur Seurch de sorte que les sites sources ne voient pas non plus votre navigateur.
