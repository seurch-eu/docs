...
> issued on **your own instance**, not on the hosted seurch.eu service.
...
```bash
curl -H "X-Api-Key: seurch_sk_<prefix>.<secret>" \
  "https://search.example.com/api/v1/web/?q=climate"
```
...
HEADERS = {"Authorization": "Api-Key seurch_sk_<prefix>.<secret>"}
...
> limit. The brand prefix `seurch_sk_` lets secret scanners flag a leaked key.
